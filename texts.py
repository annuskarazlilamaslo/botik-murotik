HELP_TEXT = (
    ...
    "📁 **Твоя папка и треки:**\n"
    "`!имя ТвоёИмя` — задать имя своей папки (можно только один раз)\n"
    "`!добавить` (или `!загрузить`, `!add`, `!upload`) — загрузить трек в свою папку "
    "(прикрепи файл вместе с командой!) 🎵\n"
    "`!треки` — посмотреть список своих треков в папке 📋\n\n"
    "🐱 **Прочее:**\n"
...
ADMIN_HELP_TEXT = (
    "🔧 **Админские команды:**\n"
    "`!админ_имя @user НовоеИмя` — сменить привязку папки\n"
    "`!админ_снести @user confirm` — снести папку пользователя со всеми треками 💥\n"
    "`!админ_треки @user` — показать треки пользователя\n"
    "`!админ_треки @user имя_файла.mp3` — удалить конкретный трек\n"
